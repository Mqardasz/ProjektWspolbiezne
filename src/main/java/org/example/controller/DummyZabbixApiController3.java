...
@RestController
@RequestMapping("/dummy-zabbix-3")
public class DummyZabbixApiController3 {
    ...
    private List<Map<String, Object>> items() {
        ...
        // Low CPU usage, high disk, heavy network – different workload profile
        double cpu  = clamp(10 + (t % 45) * 0.4 + r.nextDouble(-2, 2), 0, 100);
        double ram  = clamp(30 + (t % 180) * 0.2 + r.nextDouble(-2, 2), 0, 100);
        double disk = clamp(75 + (t % 400) * 0.02 + r.nextDouble(-0.3, 0.3), 0, 100);

        double netInBps  = clamp(1_000_000 + (t % 15) * 100_000 + r.nextDouble(-200_000, 200_000), 0, 50_000_000);
        double netOutBps = clamp(800_000   + (t % 25) * 80_000  + r.nextDouble(-150_000, 150_000), 0, 50_000_000);
